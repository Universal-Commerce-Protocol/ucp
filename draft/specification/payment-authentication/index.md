...
These Action types use the parent Checkout lifecycle defined in [Checkout — Actions](http://ucp.dev/draft/specification/checkout/#actions):
...
These are diagnostic surface codes, never authentication or payment outcomes. Following [Error Code](/draft/schemas/common/types/error_code.json), the set stays open: a surface **MAY** send another value, and a Platform **MUST** tolerate one it does not recognize and reconcile the same way.
