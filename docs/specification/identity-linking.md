...
*   MUST provide an account creation flow if the user does not already have an
    account.
*   MUST support standard UCP scopes, as defined in the Scopes section, granting
    the tokens permission to all associated Operations for a given resource.
...
*   The Platform and Merchant MAY define additional custom scopes beyond the
...
*   Revocation and security events
    ...
    *   MUST revoke the specified token and SHOULD recursively revoke all
        associated tokens (e.g., revoking a `refresh_token` MUST also
    ...
    *   SHOULD support
        ...
        account state changes initiated by the Merchant side.
...
could be required for UCP, regardless of whether the merchant supports them.
