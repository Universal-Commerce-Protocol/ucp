...
| :---- | :--- | :------- | :---------- |
| `type` | string | Yes | Provider mechanism discriminator. `oauth2` (delegated OAuth 2.0 / OIDC) and `wallet_attestation` (chain-state proof — see [Wallet Attestation](#wallet-attestation)) are defined in this version; future versions **MAY** define additional types as non-breaking extensions. |
...
| `provider_jwks` | string (URI) | Yes, for `wallet_attestation` | HTTPS URI of the provider's JWKS ([RFC 7517](https://datatracker.ietf.org/doc/html/rfc7517){ target="_blank" }) used to verify attestation signatures. See [Wallet Attestation](#wallet-attestation). |
| `attestation_endpoint` | string (URI) | No, for `wallet_attestation` | HTTPS URI where platforms POST attestation requests. See [Wallet Attestation](#wallet-attestation). |
...
Wallet attestation providers are declared under `config.providers`, keyed
by reverse-domain identifier, each key mapping to an array of mechanism
entries (see [Provider Configuration](#provider-configuration)). A
`wallet_attestation` entry is identified by its `type` discriminator:

<!-- ucp:example skip reason="capability config fragment, not a complete profile document" -->
```json
{
  "config": {
    "scopes": { },
...
Because a `wallet_attestation` entry issues no token and negotiates no
scopes, it contributes nothing to the OAuth token or scope model. Platforms
that do not support the `wallet_attestation` type **MUST** treat such entries
as filtered out (see [Provider Selection](#provider-selection)), exactly as
they would any provider `type` they do not recognize.
...
open string: `oauth2` and `wallet_attestation` (see
[Wallet Attestation](#wallet-attestation)) are defined in this version, and
the discriminator reserves space for future types — verifiable credentials
or other proof-of-identity protocols. Future versions may define additional
`type` values and the corresponding discovery and proof-presentation
mechanics.
