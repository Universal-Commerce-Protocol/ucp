...
The fulfillment extension enables merchants to advertise support for physical
...
* `methods[]` — fulfillment methods applicable to cart items (shipping, pickup, etc.)
    ...
    * `groups[]` — merchant-generated packages, each with selectable `options[]`
...
present options meaningfully. The merchant provides precomputed,
...
* Merchants SHOULD return `options[]` in a meaningful order (e.g., cheapest
    first, fastest first)
...
* Present options in the order provided by the merchant
...
Platforms MAY provide enhanced UX for recognized method types (store selectors
...
SHOULD use `continue_url` to hand off to the merchant's checkout.
...
Merchants and platforms declare fulfillment constraints in their profiles.
Merchants fetch platform profiles to adapt responses accordingly.
...
// Opt-in: merchant MAY return multiple groups per method
...
### Merchant Response Behavior

**When `supports_multi_group: false` (default):**

* Merchant MUST consolidate all items into a **single group per method**
* Response still uses array structure: `methods[].groups[]` with `groups.length === 1`
* Merchant MAY still return multiple methods (e.g., shipping + pickup) if
    cart items require it

**When `supports_multi_group: true`:**

* Merchant MAY return multiple groups per method based on inventory,
...
2. Adds corresponding merchant config options:
...
Merchant splits items into multiple packages; buyer selects shipping rate per
package.
...
**Config:** Merchant profile requires
