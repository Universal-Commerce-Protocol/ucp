...
`ready_for_complete`, `complete_in_progress`), businesses
...
*   **MAY** engage an agent to facilitate the checkout session (e.g. add items
    to the checkout session, select fulfillment address). However, the agent
    must hand over the checkout session to a trusted and deterministic UI for
    the user to review the checkout details and place the order.
...
call is the checkout object with `order_id` & `order_permalink_url` fields
populated in it. The returned `order_id` is persisted by the platform for
mirroring the state of the placed order. At the time of order persistence,
...
Businesses **SHOULD** provide all relevant links for the transaction.
The following are the recommended well-known types:
...
**SHOULD** handle unknown types gracefully by displaying them using the
`title` field or omitting them.
