...
2. **Confirm semantics.** The `consent` field is optional on requests;
   ...
   full-replacement rules (see [Update Cart](cart.md#update-cart) and
   [Update Checkout](checkout.md#update-checkout)): a business MUST retain the
   last submitted `buyer.consent` state across subsequent cart or checkout
   updates that omit `consent`, and MUST NOT reset it to advertised defaults, so
   that an update sent for an unrelated field (for example a fulfillment change)
   does not silently discard captured consent. When submitting `consent`,
