...
shared between the Platform and the Merchant.
...
Merchants advertise discount support in their profile:
...
This enables agents to explain exactly how much each discount contributed to
...
When a submitted discount code cannot be applied, merchants communicate this via
the `messages[]` array:
...
> user's expectation of the total, SHOULD use `type: "warning"` to ensure they
> are surfaced to the user rather than silently handled by agents. Rejected
> discounts are a prime example—the user expects a discount but won't receive
> it, so they should be informed.
...
Merchants may apply discounts automatically based on cart contents, customer
segment, or promotional rules:

- Appear in `discounts.applied` with `automatic: true` and no `code` field
- Applied without agent action
- Cannot be removed by the agent
- Surfaced for transparency (agent can explain to user why discount was applied)
