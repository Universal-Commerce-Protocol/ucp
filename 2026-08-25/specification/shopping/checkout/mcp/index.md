...
This document specifies the Model Context Protocol (MCP) binding for the [Checkout Capability](http://ucp.dev/2026-08-25/specification/shopping/checkout/index.md).
...
The `meta["ucp-agent"]` field is **required** on all requests to enable [capability negotiation](http://ucp.dev/2026-08-25/specification/overview/#negotiation-protocol). The `complete_checkout` and `cancel_checkout` operations also require `meta["idempotency-key"]` for retry safety. Platforms **MAY** include additional metadata fields.
...
| ------------------- | ------------------------------------------------------------------------------------------------- | -------------------------- |
| `create_checkout`   | [Create Checkout](http://ucp.dev/2026-08-25/specification/shopping/checkout/#create-checkout)     | Create a checkout session. |
| `get_checkout`      | [Get Checkout](http://ucp.dev/2026-08-25/specification/shopping/checkout/#get-checkout)           | Get a checkout session.    |
| `update_checkout`   | [Update Checkout](http://ucp.dev/2026-08-25/specification/shopping/checkout/#update-checkout)     | Update a checkout session. |
| `complete_checkout` | [Complete Checkout](http://ucp.dev/2026-08-25/specification/shopping/checkout/#complete-checkout) | Place the order.           |
| `cancel_checkout`   | [Cancel Checkout](http://ucp.dev/2026-08-25/specification/shopping/checkout/#cancel-checkout)     | Cancel a checkout session. |

### `create_checkout`

Maps to the [Create Checkout](http://ucp.dev/2026-08-25/specification/shopping/checkout/#create-checkout) operation.

#### Input Schema

- `checkout` ([Checkout](http://ucp.dev/2026-08-25/specification/shopping/checkout/#create-checkout)): **Required**. Contains the initial checkout session data and optional extensions.
  - Extensions (Optional):
    - `dev.ucp.shopping.buyer_consent`: [Buyer Consent](http://ucp.dev/2026-08-25/specification/shopping/extensions/buyer-consent/index.md)
    - `dev.ucp.shopping.fulfillment`: [Fulfillment](http://ucp.dev/2026-08-25/specification/shopping/extensions/fulfillment/index.md)
    - `dev.ucp.shopping.discount`: [Discount](http://ucp.dev/2026-08-25/specification/shopping/extensions/discount/index.md)
    - `dev.ucp.common.payment.ap2_mandate`: [AP2 Mandates](http://ucp.dev/2026-08-25/specification/payment/extensions/ap2-mandates/index.md)

#### Output Schema

- [Checkout](http://ucp.dev/2026-08-25/specification/shopping/checkout/#create-checkout) object.
  ...
  "result": {
    "structuredContent": {
      "ucp": {
        ...
        "payment_handlers": {
          "com.example.vendor.delegate_payment": [
            {"id": "handler_1", "version": "2026-08-25", "available_instruments": [{"type": "card"}], "config": {}}
          ]
        }
      },
      "id": "checkout_abc123",
...
Maps to the [Get Checkout](http://ucp.dev/2026-08-25/specification/shopping/checkout/#get-checkout) operation.
...
- [Checkout](http://ucp.dev/2026-08-25/specification/shopping/checkout/#get-checkout) object.

### `update_checkout`

Maps to the [Update Checkout](http://ucp.dev/2026-08-25/specification/shopping/checkout/#update-checkout) operation.

The Platform **MUST NOT** start a new `update_checkout` operation while the Checkout is `complete_in_progress`. Duplicate requests remain subject to [Replay Protection](http://ucp.dev/2026-08-25/specification/signatures/#replay-protection). If the Business receives a new `update_checkout` request in that state, it **MUST** leave the Checkout unchanged and return the current Checkout with a recoverable error Message.

#### Input Schema

- `id` (String): **Required**. The ID of the checkout session to update.
- `checkout` ([Checkout](http://ucp.dev/2026-08-25/specification/shopping/checkout/#update-checkout)): **Required**. Contains the updated checkout session data.
  - Extensions (Optional):
    - `dev.ucp.shopping.buyer_consent`: [Buyer Consent](http://ucp.dev/2026-08-25/specification/shopping/extensions/buyer-consent/index.md)
    - `dev.ucp.shopping.fulfillment`: [Fulfillment](http://ucp.dev/2026-08-25/specification/shopping/extensions/fulfillment/index.md)
    - `dev.ucp.shopping.discount`: [Discount](http://ucp.dev/2026-08-25/specification/shopping/extensions/discount/index.md)
...
- [Checkout](http://ucp.dev/2026-08-25/specification/shopping/checkout/#update-checkout) object.
...
Maps to the [Complete Checkout](http://ucp.dev/2026-08-25/specification/shopping/checkout/#complete-checkout) operation.
...
- `id` (String): **Required**. The ID of the checkout session.
- `checkout` ([Checkout](http://ucp.dev/2026-08-25/specification/shopping/checkout/#complete-checkout)): **Required**. Contains payment credentials and other finalization data to execute the transaction.

#### Output Schema

- [Checkout](http://ucp.dev/2026-08-25/specification/shopping/checkout/#complete-checkout) object. See core [Complete Checkout](http://ucp.dev/2026-08-25/specification/shopping/checkout/#complete-checkout) for status and `order` semantics.

### `cancel_checkout`

Maps to the [Cancel Checkout](http://ucp.dev/2026-08-25/specification/shopping/checkout/#cancel-checkout) operation.
...
- [Checkout](http://ucp.dev/2026-08-25/specification/shopping/checkout/#cancel-checkout) object with `status: canceled`.

## Error Handling

UCP distinguishes between protocol errors and business outcomes. See the [Core Specification](http://ucp.dev/2026-08-25/specification/overview/#error-handling) for the complete error code registry and transport binding examples.
...
Platforms **SHOULD** authenticate agents when using MCP transport. When using HTTP Message Signatures, all checkout operations follow the [Message Signatures](http://ucp.dev/2026-08-25/specification/signatures/index.md) specification.
...
See [Message Signatures - MCP Transport](http://ucp.dev/2026-08-25/specification/signatures/#mcp-transport) for details.
...
See [Message Signatures - REST Response Signing](http://ucp.dev/2026-08-25/specification/signatures/#rest-response-signing) for the signing algorithm (identical for MCP over HTTP).
...
1. Return errors per the [Core Specification](http://ucp.dev/2026-08-25/specification/overview/#error-handling).
...
1. Authenticate agents using one of the supported mechanisms (API keys, OAuth, mTLS, or HTTP Message Signatures per [Message Signatures](http://ucp.dev/2026-08-25/specification/signatures/index.md)).
