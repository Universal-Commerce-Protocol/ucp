...
The handler accepts [PanCredential](/2026-08-25/schemas/common/types/pan_credential.json) and [NetworkTokenCredential](/2026-08-25/schemas/common/types/network_token_credential.json) for tokenization and produces [TokenCredential](/2026-08-25/schemas/common/types/token_credential.json) for checkout.
...
Note: No `identity` is needed if the business authenticates directly—the platform knows who they are based on the API key.
...
1. The credential provider generates a token bound to the `binding` resource and issued to the business's `identity` (from the handler declaration).
...
This handler supports both Action types from the negotiated [Payment Authentication extension](http://ucp.dev/2026-08-25/specification/payment/extensions/authentication/index.md):
...
1. Call the platform's **payment credential provider** `/detokenize` endpoint with the business's `identity` alongside the `binding`.
...
Note: `identity` IS required here—the PSP is calling on behalf of a business, so they must specify which businesses' token they are retrieving.

The platform's payment credential provider verifies that:

- The PSP is authorized to act for the business named by `identity`.
- The `binding` matches the original tokenization, and `identity` resolves to the participant the token was issued to.
...
- **Pattern:** [Tokenization Payment Handler](http://ucp.dev/2026-08-25/specification/payment/guide/index.md)
- **API Pattern:** [handlers/tokenization/openapi.json](/2026-08-25/handlers/tokenization/openapi.json)
