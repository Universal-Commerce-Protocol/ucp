...
The handler schema defines three config variants for different contexts. See [Payment Handler Guide: Defining the Schema](http://ucp.dev/2026-08-25/specification/payment/guide/#defining-the-schema) for the full pattern.
...
{Include this section if the handler can cause Actions. Identify the negotiated Checkout extension that declares them. Use the standard [Payment Authentication extension](http://ucp.dev/2026-08-25/specification/payment/extensions/authentication/index.md) for device data collection and 3DS challenges; link to the defining extension specification for custom Actions.}
