...
The `prerequisites_output` contains what a participant receives from onboarding. At minimum, this includes an **identity** (see [Payment Identity](/2026-08-25/schemas/common/types/payment_identity.json)). It **MAY** also include additional configuration, credentials, or settings specific to the handler.
...
The handler declaration conforms to the [`PaymentHandler`](/2026-08-25/schemas/payment_handler.json) schema. The specification **SHOULD** define the available config and instrument schemas, and how to construct each based on the business's prerequisites output and desired configuration.
...
Each entry narrows along two axes. Both are [Constraint Expressions](http://ucp.dev/2026-08-25/specification/overview/#constraint-expression), so one evaluator runs both; they differ only in the object they describe:
...
The sibling members apply to every card instrument from this handler. The branches accept either a PAN credential carrying a `cvc` or a network token, which `network_token_credential.json` already requires to carry a `cryptogram`; a credential of any other type satisfies neither branch. Declared constraints are the upfront minimum; dynamic requirements still use recoverable errors and [`message_error.path`](/2026-08-25/schemas/common/types/message_error.json).
  ...
  "$defs": {
    ...
    "com.example.tokenizer": {
      ...
      "platform_schema": {
        ...
        "allOf": [
          { "$ref": "https://ucp.dev/2026-08-25/schemas/payment_handler.json#/$defs/platform_schema" },
          {
            "properties": {
              "config": {
...
| Schema                                                                                          | Description                                                      |
| ----------------------------------------------------------------------------------------------- | ---------------------------------------------------------------- |
| [`payment_instrument.json`](/2026-08-25/schemas/common/types/payment_instrument.json)           | Base: id, handler_id, type, billing_address, credential, display |
| [`card_payment_instrument.json`](/2026-08-25/schemas/common/types/card_payment_instrument.json) | Extends base with display: brand, last_digits, expiry, card art  |
...
An instrument schema declares what its availability constraints may name in `$defs/constraint_target`: a plain object of members and their types, never carried in a payload. The handler's `payment_instrument` set binds each `type` to the schema that owns its target. [`card_payment_instrument.json`](/2026-08-25/schemas/common/types/card_payment_instrument.json) declares `brand` as a string, so `{ "properties": { "brand": { "enum": ["visa", "mastercard"] } } }` names a declared member and pins it to string values. The base Payment Handler validates the grammar, so a malformed `constraints` fails everywhere; the target supplies the meaning, so naming an undeclared member or pinning the wrong type is reportable without the base dispatching on `type`. A handler extending an instrument **SHOULD** extend that instrument's target rather than restate it.
...
```json
{
  ...
  "$defs": {
    "constraint_target": {
      ...
      "allOf": [
        { "$ref": "https://ucp.dev/2026-08-25/schemas/common/types/card_payment_instrument.json#/$defs/constraint_target" },
        {
          "type": "object",
          "properties": {
            "tokenization_type": {
  ...
  "allOf": [
    { "$ref": "https://ucp.dev/2026-08-25/schemas/common/types/card_payment_instrument.json" }
  ],
  "type": "object",
  "required": ["type"],
  "properties": {
...
| Schema                                                                                            | Description                                 |
| ------------------------------------------------------------------------------------------------- | ------------------------------------------- |
| [`payment_credential.json`](/2026-08-25/schemas/common/types/payment_credential.json)             | Base: type discriminator only               |
| [`token_credential.json`](/2026-08-25/schemas/common/types/token_credential.json)                 | Token: type + token string                  |
| [`pan_credential.json`](/2026-08-25/schemas/common/types/pan_credential.json)                     | Raw FPAN, verified with `cvc`. Source only. |
| [`network_token_credential.json`](/2026-08-25/schemas/common/types/network_token_credential.json) | Network token, verified with `cryptogram`   |
...
```json
{
  ...
  "allOf": [
    { "$ref": "https://ucp.dev/2026-08-25/schemas/common/types/token_credential.json" }
  ],
  "type": "object",
...
See [Overview — Actions](http://ucp.dev/2026-08-25/specification/overview/#actions) for the common envelope and [Schema Authoring — Extension-Declared Action Types](/documentation/schema-authoring/#extension-declared-action-types) for the extension schema pattern.
...
**→ [Payment Handler Template](http://ucp.dev/2026-08-25/specification/payment/template/index.md)**
...
| **Link to examples**            | Reference existing handler specs and the [Tokenization Guide](http://ucp.dev/2026-08-25/specification/payment/tokenization/index.md) for common flows. |

______________________________________________________________________

## See Also

- **[Tokenization Guide](http://ucp.dev/2026-08-25/specification/payment/tokenization/index.md)** — Guide for building tokenization payment handlers
