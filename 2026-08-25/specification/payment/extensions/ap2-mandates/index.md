...
This extension uses the cryptographic primitives defined in the [Message Signatures](http://ucp.dev/2026-08-25/specification/signatures/index.md) specification:
...
- **Key Discovery:** `keys[]` in `/.well-known/ucp` (see [Key Discovery](http://ucp.dev/2026-08-25/specification/overview/#key-discovery))

See [Message Signatures](http://ucp.dev/2026-08-25/specification/signatures/index.md) for key format and rotation.
...
Verification and mandate construction operate on the complete checkout JSON, not a projection of schema-recognized fields; removing any member covered by `merchant_authorization` changes the JCS payload and invalidates the signature. This coverage includes `ucp` protocol-namespace members present in the checkout; only the `ap2` field is excluded as specified below.
