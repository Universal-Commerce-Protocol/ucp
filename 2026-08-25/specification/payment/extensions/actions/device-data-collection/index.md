...
This specification defines the device data collection Action type declared by the [Payment Authentication extension](http://ucp.dev/2026-08-25/specification/payment/extensions/authentication/index.md):
...
The config shape is defined inline by the [Payment Authentication extension schema](/2026-08-25/schemas/common/payment_authentication.json).
...
Mounting the surface **MUST** follow the shared [Payment Authentication rendering contract](http://ucp.dev/2026-08-25/specification/payment/extensions/authentication/#surface-rendering-and-notifications) and [Embedded Protocol security requirements](http://ucp.dev/2026-08-25/specification/embedded-protocol/#security).
...
After completing the shared [`action.ready` handshake](http://ucp.dev/2026-08-25/specification/payment/extensions/authentication/#ready-handshake), the surface sends JSON-RPC 2.0 notifications defined by the [payment Action embedded contract](http://ucp.dev/2026-08-25/specification/payment/extensions/authentication/#surface-rendering-and-notifications).
...
Surface-level codes are the shared [well-known `action.error` codes](http://ucp.dev/2026-08-25/specification/payment/extensions/authentication/#surface-rendering-and-notifications). They are not EMV 3DS or payment outcomes: a provider-domain timeout or unavailable method that the handler can safely continue past should normally result in `action.done`, with the Business and provider recording the domain outcome server-side. After either notification, the Platform follows the shared Payment Authentication reconciliation contract.
...
The common [Payment Authentication security requirements](http://ucp.dev/2026-08-25/specification/payment/extensions/authentication/#security-and-data-handling) apply. In addition:
